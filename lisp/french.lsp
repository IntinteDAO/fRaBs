...
(setq single_play     "    Sortir du jeu sur r‚seau    ")  ; V-A 
	 ...
	 (setq plot_middle
        ...
        ('upgrade_section
         (setq enter_old_dir   "Entrez le r‚pertoire de la version commerciale ult‚rieure d'ABUSE")
         (setq dir_not_here    "Le r‚pertoire n'existe pas : ")
         (setq already_updated "Cette version d'ABUSE est d‚j… mise … jour")
         (setq updating        "Mise … jour des fichiers, veuillez attendre")
         (setq yes-key         "O")
         (setq no-key          "N")
         (setq complete_title  "La nouvelle version shareware a ‚t‚ mise … jour")
         (setq remove_old_reg  "Effacer l'ancienne version commerciale ? (O/N)"))
)
