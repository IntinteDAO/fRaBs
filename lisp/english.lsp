...
(select section	
	('game_section
	 ...
	 (setq no_low_mem         (concatenate 'string "Memory Manager : Not enough low memory available\n"
					   "  Suggestions...\n"
					   "    - make a boot disk (consult manual)\n"
                                           "    - remove TSRs  & drivers not needed by ABUSE\n"
					   "    - add memory to your system\n"))
	 (setq no_mem    (concatenate 'string "Memory manager : Sorry you do not have enough memory available to\n"
					   "  Suggestions...\n"
					   "    - make a boot disk (consult manual)\n"
                                           "    - remove TSRs  & drivers not needed by ABUSE\n"
					   "    - add memory to your system\n"))
         ...
         (if (not (load "register/english.lsp"))
             (setq end_msg 
                   (concatenate 'string 
                                ...
                                "* Look for the commercial version of ABUSE in stores late March '96 for \n"
                                "  Eight Player IPX Network Support, a Built-in Level Editor, 17 Single Player \n"
                                "  Levels (plus 12 Network-Only Levels), Deadlier Weapons, Extra Powerups, \n"
                                "  Stronger Enemies, Cooler Sound Effects, and More Artwork.\n\n"

                                "* To order the full version of ABUSE from Electronic Arts, please call:\n"
                                "  USA       1-800-245-4525\n"
                                "  CANADA    604-451-3600\n"
                                "  ENGLAND   0753-549442\n"
                                "  FRANCE    72 17 07 83\n"
                                "  GERMANY   05241 / 24307\n"
                                "  AUSTRALIA +61(75)911 388\n"
                                "  JAPAN     03-5410-3100\n\n")))
	...
	('upgrade_section
	 (setq enter_old_dir   "Enter directory of the older registered version of ABUSE")
         (setq dir_not_here    "Directory does not exsist : ")
	 (setq already_updated "This version of ABUSE is already up to date")
	 (setq updating        "Updating files, please wait")
	 (setq yes-key         "Y")
	 (setq no-key          "N")
	 (setq complete_title  "New shareware version has been upgraded")
	 (setq remove_old_reg  "Delete old registered version? (Y/N)"))
)
